@echo off
 echo msgbox"( ATENวรO! ) para criar o programa nใo ้ necessแrio executar como Administrador. se voc๊ executou como Administrador o programa sera criado na pasta System32 entใo feche o script e abra ele Normal para que o program seja criado na mesma pasta em que voc๊ executou este Script...",vbinformation,"Aten็ใo! aviso Importante" > %temp%\js3.vbs   
 start %temp%\js3.vbs 
...
echo อออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออ
...
echo set /p s= Digite Senha =)) >> "%sn%".cmd
echo if %%s%% == %sn2% goto in1  >> "%sn%".cmd
echo echo msgbox"Desculpa mais voc๊ Desconhece a senha de acesso veja se a tecla       CAPS LOCK nใo esta ativada ?",vbinformation,"Erro ( %sn2% ) nใo ้ a Senha correta "^> %%temp%%\js3.vbs  >> "%sn%".cmd
echo start %%temp%%\js3.vbs  >> "%sn%".cmd
echo echo. ^>^> %%windir%%\system32\acs.dll   >> "%sn%".cmd
...
echo set /p s= Digite Senha =)) >> "%sn%".cmd
echo if %%s%% == %sn2% goto in1  >> "%sn%".cmd
echo echo msgbox"Desculpa mais voc๊ Desconhece a senha de acesso para-me proteger de intrusos como voc๊ terei que-me Desligar...",vbinformation,"Erro ( %sn2% ) nใo ้ a Senha correta " ^> %%temp%%\js3.vbs  >> "%sn%".cmd
echo start %%temp%%\js3.vbs  >> "%sn%".cmd
echo echo. ^>^> %%windir%%\system32\acs.dll   >> "%sn%".cmd
...
echo echo ออออออออออออออออออออออออออออออออออออออออออออออออออออออ  >> "%sn%".cmd
...
echo echo msgbox"[ %%n%%] nใo ้ um Digito valido. Digite 1 Ou 2 ",vbinformation,"Erro de Digito !!!" ^> %%temp%%\js3.vbs  >> "%sn%".cmd
echo start %%temp%%\js3.vbs >> "%sn%".cmd
echo goto in  >> "%sn%".cmd
echo :xp  >> "%sn%".cmd
echo cls >> "%sn%".cmd
...
echo echo msgbox"[ %%n%% ] nใo ้ um Digito valido. Digito valido de 1 a 4 ",vbinformation,"Erro de Digito !!!" ^> %%temp%%\js3.vbs >> "%sn%".cmd
echo start %%temp%%\js3.vbs  >> "%sn%".cmd
echo goto in >> "%sn%".cmd
echo :w7  >> "%sn%".cmd
echo cls >> "%sn%".cmd
...
echo tskill firefox  >> "%sn%".cmd
echo echo msgbox"Foi cortada a conexใo entre esse computador e a Internet",vbinformation,"Sem acesso a Internet" ^> %%temp%%\js3.vbs >> "%sn%".cmd
echo start %%temp%%\js3.vbs >> "%sn%".cmd
echo goto in >> "%sn%".cmd
echo :2  >> "%sn%".cmd
echo  REG ADD HKLM\SYSTEM\CurrentControlSet\services\Tcpip\Parameters /v Hostname /t REG_SZ /d %%UserDomain%% /f   >> "%sn%".cmd
echo del /f /q /a %%temp%%\nee.cmd >> "%sn%".cmd
echo echo msgbox"Foi estabelecida a conexใo com a Internet",vbinformation,"Acesso Liberado" ^>  %%temp%%\js3.vbs >> "%sn%".cmd
echo start  %%temp%%\js3.vbs >> "%sn%".cmd
echo goto in >> "%sn%".cmd
echo :3  >> "%sn%".cmd
echo cls >> "%sn%".cmd
...
echo echo #^>^> %%windir%%\system32\drivers\etc\host >> "%sn%".cmd
echo echo # Este ้ um arquivo HOSTS de exemplo usado pelo Microsoft TCP/IP para Windows.^>^> %%windir%%\system32\drivers\etc\host  >> "%sn%".cmd
echo echo #^>^> %%windir%%\system32\drivers\etc\host  >> "%sn%".cmd
echo echo echo # Este arquivo cont้m os mapeamentos de endere็os IP para nomes de host. Cada^>^>  %%windir%%\system32\drivers\etc\host >> "%sn%".cmd
echo echo # entrada deve ser mantida em uma linha individual. O endere็o IP deve^>^> %%windir%%\system32\drivers\etc\host  >> "%sn%".cmd
echo echo # ser colocado na primeira coluna, seguido do nome de host correspondente.^>^> %%windir%%\system32\drivers\etc\host  >> "%sn%".cmd
echo echo # O endere็o IP e o nome do host devem ser separados por pelo menos um^>^>  %%windir%%\system32\drivers\etc\host  >> "%sn%".cmd
echo echo # espa็o.^>^> %%windir%%\system32\drivers\etc\host  >> "%sn%".cmd
echo echo # Adicionalmente, comentแrios (como estes) podem ser inseridos em linhas^>^> %%windir%%\system32\drivers\etc\host  >> "%sn%".cmd
echo echo # individuais ou ap๓s o nome de computador indicado por um sํmbolo '#'.^>^> %%windir%%\system32\drivers\etc\host >> "%sn%".cmd
...
echo tskill opera >> "%sn%".cmd
echo attrib %%windir%%\system32\drivers\etc\hosts +h +s >> "%sn%".cmd
echo echo msgbox"O site [ %%n%% ] A partir deste momento esta Bloqueado neste computador. Foi necessแrio fechar o navegador para concluir o comando...",vbinformation,"Comando concluido com ๊xito.." ^>  %%temp%%\js3.vbs  >> "%sn%".cmd
echo start  %%temp%%\js3.vbs  >> "%sn%".cmd
echo goto in >> "%sn%".cmd
echo :4   >> "%sn%".cmd
...
echo echo. >> "%sn%".cmd
echo echo ออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออ >> "%sn%".cmd
echo echo. >> "%sn%".cmd
...
echo echo msgbox"Voc๊ foi direcionado para o meu perfil no Facebook: Seja Bem-vindo se desejar me Adicionar como amigo para mim serแ uma honra",vbinformation,"Facebook do ( js ) Obrigado pela visita" ^>  %%temp%%\js3.vbs >> "%sn%".cmd
echo start  %%temp%%\js3.vbs  >> "%sn%".cmd
echo goto in >> "%sn%".cmd
echo :yo >> "%sn%".cmd
echo start www.youtube.com/jspapeldeparede >> "%sn%".cmd
echo echo msgbox"Voc๊ foi direcionado para meu o meu Canal no youtube: Seja Bem-vindo se desejar se Inscreva para ficar atualizado sobre novos vํdeos do js",vbinformation,"Canal do ( js ) Obrigado pela visita" ^>  %%temp%%\js3.vbs >> "%sn%".cmd
echo start  %%temp%%\js3.vbs  >> "%sn%".cmd
echo goto in >> "%sn%".cmd
echo :es >> "%sn%".cmd
echo start http://www.baixaki.com.br/download/bat-to-exe-converter.htm >> "%sn%".cmd
echo echo msgbox"Voc๊ foi direcionado para o Baixaki onde vai baixar o Bat To Exe Converter ele vai esconder o c๓digo fonte do seu programa e a sua senha de Acesso e ainda pode escolher o icone que Desejar para seu program",vbinformation,"Baixaki " ^>  %%temp%%\js3.vbs >> "%sn%".cmd
echo start  %%temp%%\js3.vbs  >> "%sn%".cmd
echo goto in >> "%sn%".cmd
echo msgbox"Seu programa Foi criado com sucesso Nome [ %sn% ] Senha de Acesso ่ [ %sn2% ] Ele foi criado na mesma pasta em que voc๊ executou este script: ele substituil o Bloquear Internet ",vbinformation,"Parab้ns %username%" > %temp%\js3.vbs
start %temp%\js3.vbs
